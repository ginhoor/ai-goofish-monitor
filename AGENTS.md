...
4. 合并前必须确认：范围单一；无未提交、未跟踪或无关改动；关键验证通过；必要文档和进度已回填；功能分支已 push；已同步最新 `origin/master` 并解决冲突。
5. 不可逆迁移、线上数据修改、凭证轮换、部署状态变化、分支保护或必需人工审批不因本节获得额外授权；此时必须保持未完成/阻塞并请求用户决定。
6. 安全门禁满足后，Agent 必须主动合并到 `master` 并 push。使用 GitHub PR 时保持 `delete_branch_on_merge=true`，默认采用 `gh pr merge --delete-branch` 或等价方式。
7. 合并后验证 `master` 与 `origin/master` 已包含目标变更。任务 worktree 先确认无未保存内容和独有提交，再使用 `git worktree remove <path>` 正常移除；禁止直接删除已注册目录。
8. 删除已合并的本地和远端任务分支，并验证物理目录、worktree 注册、本地 ref、远端 head 和 prune 后的 remote-tracking ref 全部消失。
